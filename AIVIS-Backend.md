...
>
> **H12 (2026-09):** `KYCApplicationStatus` снесён — вокабуляр статусов теперь один,
> `KYCStatus` в `users/models.py`, и оба CHECK-констрейнта сверяются с ним тестом
> в `tests/test_kyc.py`. Заявка перестала быть пустой: `POST /kyc/submit` принимает
> multipart с документами, они лежат в MinIO под префиксом `kyc/applications/`
> и описаны строками `kyc_documents`. Перечисленные ниже имена файлов и функций
> читать как состояние на момент спринта 2.1, не как текущее.
...
| TD-080 | `backend/pyproject.toml` | Незапиненный `fastapi>=0.115,<1.0` без lock-файла: пересборка образа тихо подтянула 0.139.2 с breaking-изменением в `include_router` (0.137+ — routes под-роутера не флэттенятся в `app.routes`, а прячутся за ленивой обёрткой `_IncludedRouter`) → сломался route-walk мета-тест `test_avatar`. Интерим-фикс применён: потолок `<0.140` (пропускает протестированную 0.139.x, блокирует следующий минор). Настоящий фикс: lock-файл (`uv.lock` / pip-tools `requirements.txt` с хешами) для воспроизводимой сборки — апгрейды осознанные, а не молчаливые. | Before Scale | ⬜ (pin applied, lock open) |
| TD-081 | `users/router.py` (PUT `/users/me/payout-details`) | Эндпоинт принимает произвольный JSON в `payout_details` без валидации/whitelisting ключей (тот же эндпоинт использует CompanyBalanceView). Фронт (AgentSettingsView §6) уже отвергает пустой `{}`, но неизвестные/мусорные ключи проходят на бэк. Фикс: валидировать/игнорить неизвестные ключи серверно (Pydantic-схема payout-реквизитов или явный allow-list). Review §4, пре-существующее (общий эндпоинт), не F6-регрессия. | Before Prod | ⬜ |
...
*Version 3.9-F6 | 2026-07-25 | Code-review round-2 (backend slice). `get_commission_month_to_date`: **include volume_bonus** в агрегат «Commission this month» (был `commission:`-only — молча сузил цифру виджета относительно удалённого клиентского подсчёта; reversed по-прежнему исключён); лживый докстринг «parity with tile» переписан (review 2.5). `get_my_commissions`: id tie-breaker на `DESC created_at` — стабильная offset-пагинация при равном транзакционном `created_at` (`func.now()`), что новый `:key="entry.id"` делал видимым как Vue dup-warning (2.6). Тесты: volume_bonus теперь суммируется (3000→3700) + тест стабильности пагинации. 313 tests green.*

*Version 3.8-F6 | 2026-07-24 | F6 review follow-up (backend slice) + test-suite hardening. `CommissionEntry.id` выставлен на `GET /agent/commissions/me` (stable ledger PK для client list keys) + новый `tests/test_commissions_list.py` (id / типы commission+volume_bonus / reversed listed / non-agent 403). Test fixes: `test_regression_guards` L3 сделан герметичным (per-run unique lang вместо shared `SYNTHETIC_LANG` — обходит tie-break-less `ORDER BY priority LIMIT 1` в `find_active_template` при residue-строках на общей БД); `test_avatar` route-walk сделан рекурсивным под FastAPI 0.137+ `_IncludedRouter` (гварды на роутах на месте и работают в рантайме — мета-тест давал false-negative из-за версионного дрейфа зависимости). `fastapi` capped `<0.140` (TD-080). TD-075 (N+1 в get_my_commissions) подтверждён закрытым — batched preload в коде. 312 tests green.*
