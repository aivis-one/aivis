...
<template>
  <div class="c-empty">
    <div class="c-empty__icon"><slot name="icon" /></div>
    <div v-if="title" class="c-empty__title">{{ title }}</div>
    <div v-if="description" class="c-empty__desc">{{ description }}</div>
  </div>
</template>
...
.c-empty__desc { max-width: var(--maxw-prose); }
</style>
