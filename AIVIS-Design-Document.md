...
AIVIS.ONE -- платформа для четырёх аудиторий, объединённая под одним доменом `aivis.one`:
...
- Принципы документа, в рамках которого принимается решение (например, P-OBS-1...P-OBS-5 в специализированном доменном документе, или другие, появляющиеся в специализированных документах по мере их написания).
...
6. Nginx reverse proxy (`api.aivis.one`, `app.aivis.one`) + SSL
...
Таблица регистрировала **3 из 10** документов `AIVIS-*.md`, лежащих в корне репозитория.
Актуализирована 2026-08-20: зарегистрированы все десять, с размером и датой последнего
коммита каждого — по этим двум полям запись можно перепроверить, в отличие от слова
«актуален», которое ничем не подтверждается.

**Колонка «Проверен» — это НЕ оценка содержания.** Она говорит ровно одно: читал ли
кто-то документ целиком при последней сверке. `нет` означает «никто не подтверждал»,
а не «устарел».

| Документ | Путь | Размер | Строк | Последний коммит | Проверен целиком |
|---|---|---:|---:|---|---|
| Конституция (этот документ) | `AIVIS-Design-Document.md` | 45 650 B | 715 | 2026-08-03 `bbdb321` | нет |
| Бэковый Кодекс | `AIVIS-Backend.md` | 276 732 B | 3 602 | 2026-08-04 `c57594c` | нет |
| Фронтовый Кодекс | `AIVIS-Frontend.md` | 193 275 B | 2 313 | 2026-08-05 `a10fd01` | нет |
| Рефактор: документы компаний | `AIVIS-Refactor-Company-Docs.md` | 106 902 B | 1 094 | 2026-08-03 `de1d92e` | нет |
| Рефактор: инвестор, рынок, стафф | `AIVIS-Refactor-Investor-Market-And-Staff.md` | 57 649 B | 694 | 2026-07-26 `f6e5144` | нет |
| Раннбук деплоя | `AIVIS-Deploy-Runbook.md` | 44 249 B | 618 | 2026-08-20 `18d0059` | нет |
| Финансовая система | `AIVIS-Financial-System.md` | 33 265 B | 676 | 2026-07-26 `f6e5144` | нет |
| Рефактор пула долей | `AIVIS-Share-Pool-Refactor.md` | 32 655 B | 735 | 2026-07-26 `f6e5144` | нет |
| Рассрочка | `AIVIS-Installment.md` | 15 022 B | 349 | 2026-07-26 `f6e5144` | нет |
| Машины состояний | `AIVIS-State-Machines.md` | 12 198 B | 309 | 2026-07-26 `f6e5144` | нет |
| Справочник дизайн-системы (ГЕНЕРИРУЕТСЯ) | `AIVIS-Design-System.md` | 21 587 B | 536 | 2026-08-20 создан | из исходников |
| Install script | `scripts/install_aivis.sh` | — | — | активно меняется | — |
| Notification templates | `backend/app/modules/notifications/templates/en.yaml` | — | — | MVP | — |
| Схема БД (правда) | `backend/app/modules/*/models.py` | — | — | источник истины | — |

**Итого одиннадцати документов: 839 184 B / 11 641 строка** (измерено 2026-08-20 в БАЙТАХ, с вычтенными CR).
**⚠ «Четыре дизайн-документа», как их называет реестр открытых работ, — это ПОДМНОЖЕСТВО:**
`AIVIS-Backend.md` + `AIVIS-Frontend.md` + `AIVIS-Design-Document.md` +
`AIVIS-Refactor-Company-Docs.md` = 622 559 B / 7 724 строки. Цифра верна, но считает
четыре документа из десяти — **называйте, какое множество имеете в виду.**

**✅ ПРОБЕЛ ЗАКРЫТ 2026-08-20.** Ни один из десяти документов не перечислял, какие компоненты и
токены есть в наличии — это приходилось выяснять чтением `frontend/src/components/ui/` и
`variables.css`. Теперь есть `AIVIS-Design-System.md`, и он **генерируется из исходников**:
`npm --prefix frontend run docs:ds`. **Править его руками нельзя — правки затрутся при следующем
прогоне; менять надо исходник.** Так он не может разойтись с кодом — в отличие от этой таблицы,
которая расходилась.
...
*Version 1.6 | 2026-05-12 | Запись добавлена задним числом. Шапка документа объявляет версию 1.6, но описание изменений 1.6 не было зафиксировано нигде — сама запись 1.5 при этом ранее была ошибочно перезаписана на 1.6 и здесь восстановлена.*
