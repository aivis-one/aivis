...
**Итого десяти документов: 817 597 B / 11 105 строк** (измерено 2026-08-20 в БАЙТАХ, с вычтенными CR).
...
**⚠ ЧЕГО ЗДЕСЬ НЕТ И ЧТО ЭТО ЗНАЧИТ: справочника по дизайн-системе фронтенда.** Ни один из
десяти документов не перечисляет, какие компоненты и токены существуют в наличии. Кто хочет
это узнать, читает `frontend/src/components/ui/` и `frontend/src/styles/variables.css`
напрямую. Зарегистрировано как пробел, не закрыто.
