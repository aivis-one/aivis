...
> **⚠ Документ заморожен и не обновляется.** Доменные имена в нём — из эпохи до ребрендинга и не отражают
> текущие домены продукта. Актуальное описание живых механизмов (MinIO console nginx proxy, `aivis storage
> console`, reconcile workflow) — в `AIVIS-Deploy-Runbook.md`.
