...
# ==============================================================================
# FRONTEND FONTS (Self-hosted)
# ==============================================================================
# Downloads Montserrat + Noto Sans Arabic from Google Fonts and saves them
# to frontend/public/fonts/. Generates frontend/src/styles/fonts.css with
# @font-face rules pointing to /fonts/ instead of fonts.gstatic.com.
# This eliminates the dependency on Google Fonts CDN at runtime.
# ==============================================================================

section "Frontend Fonts (Self-hosted)"

FONTS_DIR="$INSTALL_BASE/repo/frontend/public/fonts"
FONTS_CSS="$INSTALL_BASE/repo/frontend/src/styles/fonts.css"
FONT_UA="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
FONT_API="https://fonts.googleapis.com/css2?family=Montserrat:ital,wght@0,400;0,500;0,600;0,700;0,800;1,400;1,500;1,600;1,700;1,800&family=Noto+Sans+Arabic:wght@400;500;600;700&display=swap"

mkdir -p "$FONTS_DIR"

log "Fetching font manifests from Google Fonts..."
COMBINED_CSS=$(curl -fsSL -A "$FONT_UA" "$FONT_API") || error "Failed to fetch Google Fonts CSS"

FONT_COUNT=$(echo "$COMBINED_CSS" | grep -oP 'https://fonts\.gstatic\.com/s/[^)]+\.woff2' | sort -u | wc -l)
log "Found $FONT_COUNT woff2 files to download..."

echo "$COMBINED_CSS" | grep -oP 'https://fonts\.gstatic\.com/s/[^)]+\.woff2' | sort -u | \
while read -r URL; do
    FNAME=$(basename "$URL")
    ...
    if [ -f "$DEST" ]; then
        ...
        curl -fsSL -A "$FONT_UA" "$URL" -o "$DEST" && echo "  OK $FNAME" || warn "Failed to download $FNAME"
    fi
done

log "Generating fonts.css..."
# Replace each gstatic URL with a local /fonts/<basename> path.
{
    echo "/* Self-hosted fonts -- generated by install_cbshome.sh */"
    echo "/* Do not edit manually. Re-run install to regenerate. */"
    echo ""
    echo "$COMBINED_CSS" | sed 's|url(https://fonts\.gstatic\.com/s/[^)]*\/\([^/)]*\.woff2\))|url(/fonts/\1)|g'
} > "$FONTS_CSS"

DOWNLOADED=$(ls "$FONTS_DIR"/*.woff2 2>/dev/null | wc -l)
success "Fonts self-hosted: $DOWNLOADED woff2 files -> $FONTS_DIR"
success "fonts.css generated: $FONTS_CSS"
