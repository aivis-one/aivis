...
def _resolve_type(prop: dict) -> str:
    """Convert a single OpenAPI property definition to a TypeScript type
    string.
...
def generate(openapi: dict) -> str:
    """Generate complete TypeScript file from an OpenAPI schema dict."""
    ...
    bar = "// " + "=" * 77
    output.append(bar)
    output.append(f"// {title} -- Auto-generated TypeScript types")
    output.append(bar)
    output.append("//")
    ...
    # -- Emit interfaces --
    if interface_names:
        output.append("// -- Interfaces " + "-" * 63)
        output.append("")
        for name in interface_names:
            output.extend(_generate_interface(name, schemas[name]))
