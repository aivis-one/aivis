...
@pytest.fixture(autouse=True)
async def assert_seed_invariants() -> AsyncGenerator[None, None]:
    """Fail fast if a previous test mutated seed-state cardinality.

    The template DB ships with:
      - exactly 1 User with role=PLATFORM,
      - exactly 16 platform-default CompanyDocumentTemplate rows
        (company_id IS NULL, status=active).

    If a test commits a change that breaks either invariant and forgets
    to restore it, ALL subsequent tests in this worker that hit
    get_platform_user_id() or find_active_template() would cascade-fail
    with confusing MultipleResultsFound / template_missing errors -- the
    way 279 failures in one run obscured a single mutation in
    test_login_platform_user. This fixture turns that 279-failure cascade
    into one obvious error in the FIRST test that runs against broken
    state, pointing at the actual offender.

    Runs BEFORE every test (the yield is at the end intentionally -- the
    check guards entry into the next test, not exit from the previous
    one, which is what catches the regression). The fixture itself is
    cheap: two scalar SELECTs against a per-worker DB.
    """
    from sqlalchemy import func, select

    from app.core.database import get_session_factory
    from app.modules.companies.constants import TemplateStatus
    from app.modules.companies.models import CompanyDocumentTemplate
    from app.modules.users.models import User, UserRole

    factory = get_session_factory()
    async with factory() as sanity_session:
        # Platform user cardinality.
        platform_count = (
            await sanity_session.execute(
                select(func.count(User.id)).where(User.role == UserRole.PLATFORM)
            )
        ).scalar_one()
        assert platform_count == 1, (
            f"Seed invariant violated: expected exactly 1 Platform user, "
            f"found {platform_count}. A previous test mutated User.role and "
            "did not revert it. Re-run with -p no:xdist to identify the "
            "offending test by name."
        )

        # Platform-default templates cardinality.
        ...
        assert platform_template_count == 16, (
            f"Seed invariant violated: expected exactly 16 active platform-"
            f"default CompanyDocumentTemplate rows (company_id IS NULL), "
            f"found {platform_template_count}. A previous test mutated "
            "platform-default templates and did not restore them."
        )
