...
class Settings(BaseSettings):
    ...
    # -- Email --
    #
    # THERE ARE NO EMAIL SETTINGS HERE ANY MORE, ON PURPOSE. This
    # product does not send email; the comms notification service does,
    # and it holds the provider credentials in its own env under its own
    # names (EMAIL_MAILGUN_API_KEY / EMAIL_MAILGUN_DOMAIN /
    # EMAIL_FROM_ADDRESS / EMAIL_MAILGUN_REGION). Nine settings lived
    # here -- six SMTP, three Mailgun -- read by exactly one module,
    # core/email.py, which this delivery removed along with the last
    # local MTA.
    #
    # backend/.env STILL CARRIES MAILGUN_API_KEY / MAILGUN_DOMAIN /
    # MAILGUN_API_URL / SMTP_FROM_EMAIL, and that is not a leftover:
    # they are the hand-over source the installer reads to deliver the
    # four comms keys, the same arrangement TELEGRAM_BOT_URL has.
    # Settings ignores unknown env keys, so nothing here needs to
    # declare them.
    #
    # HIGH_SECURED_DOMAINS went with them. It was already dead -- its
    # routing logic was deleted 2026-08-14 and the property that parsed
