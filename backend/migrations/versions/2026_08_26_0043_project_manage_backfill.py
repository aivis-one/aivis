...
WHY THIS REVISION ID IS SHORTER THAN THE FILE NAME USED TO BE. The id
started life as "0043_project_manage_admin_backfill" -- 34 characters --
and alembic_version.version_num is VARCHAR(32) by alembic's default,
which migrations/env.py did not override. So this migration could not
record itself: the backfill ran, the version UPDATE raised
StringDataRightTruncationError, and the whole step rolled back on every
box. It never applied anywhere, which is exactly why renaming the id is
safe here and would not be safe for a migration that had.

The 32-character ceiling itself is lifted by 0045_alembic_version_width,
which runs after this one. Ids at or before that migration must stay
within 32 characters; ids after it may use 64. A guard in
tests/test_regression_guards.py enforces that split rather than trusting
...
revision: str = "0043_project_manage_backfill"
